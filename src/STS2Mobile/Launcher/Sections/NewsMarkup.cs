...
public static class NewsMarkup
{
    public static string ToGodotBbcode(string source)
    {
        ...
        text = Regex.Replace(text, @"\[h([1-6])\](.*?)\[/h\1\]", "\n[b][font_size=26]$2[/font_size][/b]\n",
        ...
        text = Regex.Replace(text, @"\[img\].*?\[/img\]", "", RegexOptions.Singleline | RegexOptions.IgnoreCase);
    ...
    private static string ConvertHtml(string text)
    {
        ...
        text = Regex.Replace(text, @"<(strong|b)>(.*?)</\1>", "[b]$2[/b]",
            RegexOptions.Singleline | RegexOptions.IgnoreCase);
        text = Regex.Replace(text, @"<(em|i)>(.*?)</\1>", "[i]$2[/i]",
            RegexOptions.Singleline | RegexOptions.IgnoreCase);
        text = Regex.Replace(text, @"<a[^>]*href=""([^""]*)""[^>]*>(.*?)</a>", "[url=$1]$2[/url]",
            RegexOptions.Singleline | RegexOptions.IgnoreCase);
